...
class AGWindowMSA(BaseModule):
    ...
    def __init__(self,
        ...
        self.q = nn.Linear(
            embed_dims + appearance_dims, embed_dims, bias=qkv_bias)
        self.k = nn.Linear(
            embed_dims + appearance_dims, embed_dims, bias=qkv_bias)
    ...
    def _add_lora(self, device, r=1, alpha:Optional[int]=1, requires_grad=True):
        rank = r
        self.gamma = alpha / rank ** (1/2)
        self.added_lora = True

        self.w_a_q = Parameter(torch.empty((r, self.embed_dims + self.appearance_dims)).to(device))
        self.w_b_q = Parameter(torch.empty((self.embed_dims, r)).to(device))
        self.w_a_k = Parameter(torch.empty((r, self.embed_dims + self.appearance_dims)).to(device))
        self.w_b_k = Parameter(torch.empty((self.embed_dims, r)).to(device))
        self.w_a_v = Parameter(torch.empty((r, self.embed_dims)).to(device))
        self.w_b_v = Parameter(torch.empty((self.embed_dims, r)).to(device))

        self._reset_lora()

        if not requires_grad:
                self.w_a_q.requires_grad = False
                self.w_b_q.requires_grad = False
                self.w_a_k.requires_grad = False
                self.w_b_k.requires_grad = False
                self.w_a_v.requires_grad = False
                self.w_b_v.requires_grad = False

    def _reset_lora(self):
        
        kaiming_uniform_(self.w_a_q, a=math.sqrt(5))
        zeros_(self.w_b_q)
        kaiming_uniform_(self.w_a_k, a=math.sqrt(5))
        zeros_(self.w_b_k)
        kaiming_uniform_(self.w_a_v, a=math.sqrt(5))
        zeros_(self.w_b_v)
    ...
    def forward(self, x, mask=None):
        ...
        if not self.added_lora:
            ...
            q = (self.q(x) + F.linear(x, self.gamma * F.linear(self.w_b_q, self.w_a_q.T, None), None)).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            k = (self.k(x)+ F.linear(x, self.gamma * F.linear(self.w_b_k, self.w_a_k.T, None), None)).reshape(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            v = (self.v(x[:, :, :self.embed_dims]) + F.linear(x[:, :, :self.embed_dims], self.gamma * F.linear(self.w_b_v, self.w_a_v.T, None), None)).reshape(
                B, N, self.num_heads, self.embed_dims // self.num_heads).permute(
                    0, 2, 1, 3)  # B NUM_HEADS N embed_dims//NUM_HEADS
...
class AttentionLayer(nn.Module):
    ...
    def _add_lora(self, device, r=1, alpha:Optional[int]=1, requires_grad=True):
        ...
        self.w_a_q = Parameter(torch.empty((r, self.hidden_dim + self.guidance_dim)).to(device))
        self.w_b_q = Parameter(torch.empty((self.hidden_dim, r)).to(device))
        self.w_a_k = Parameter(torch.empty((r, self.hidden_dim + self.guidance_dim)).to(device))
        self.w_b_k = Parameter(torch.empty((self.hidden_dim, r)).to(device))
        self.w_a_v = Parameter(torch.empty((r, self.hidden_dim)).to(device))
        self.w_b_v = Parameter(torch.empty((self.hidden_dim, r)).to(device))
    ...
    def forward(self, x, guidance=None):
        ...
        if not self.added_lora:
            ...
            q = self.q(torch.cat([x, guidance],
                                dim=-1)) + F.linear(torch.cat([x, guidance], dim=-1), self.gamma * F.linear(self.w_b_q, self.w_a_q.T, None), None) if guidance is not None else self.q(x) + F.linear(x, self.gamma * F.linear(self.w_b_q, self.w_a_q.T, None), None)
            k = self.k(torch.cat([x, guidance],
                                dim=-1)) + F.linear(torch.cat([x, guidance], dim=-1), self.gamma * F.linear(self.w_b_k, self.w_a_k.T, None), None) if guidance is not None else self.k(x) + F.linear(x, self.gamma * F.linear(self.w_b_k, self.w_a_k.T, None), None)
            v = self.v(x) + F.linear(x, self.gamma * F.linear(self.w_b_v, self.w_a_v.T, None), None)
...
class CATSegAggregator(BaseModule):
    ...
    def forward(self, inputs, latent=False):
        ...
        # fix any bug
        if text_feats.shape[0] == B and text_feats.shape[2] != 80:
            text_feats = text_feats.repeat(1, 1, 8, 1)
        elif text_feats.shape[0] == B and text_feats.shape[2] == 80:
            pass
        elif text_feats.shape[1] == 80:
            text_feats = text_feats.repeat(B, 1, 1, 1)
        else:
            text_feats = text_feats.unsqueeze(0) # changed for hyperlora with different number of prompts from index 1 to index 0
            text_feats = text_feats.repeat(B, 1, 8, 1) # changed for hyperlora with 10 prompts
        ...
        if self.topk_templates is not None:
            _, topk_templates = torch.topk(torch.mean(corr, dim=(2, 3, 4)), 40, dim=1)
        else:
            topk_templates = None
        ...
        if latent:
            return dict(
                corr_embed=corr_embed, appearance_feats=appearance_guidance[1:]), cost_volume
        else:
            return dict(
                corr_embed=corr_embed, appearance_feats=appearance_guidance[1:], topk_templates=topk_templates)
