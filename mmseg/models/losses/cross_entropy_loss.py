...
def thresh_cross_entropy(pred,
    ...
    prob, lbl = torch.max(softmax, dim=1)

    if isinstance(threshold, torch.Tensor):
        prob = prob.clone().cpu()
        lbl = lbl.clone().cpu()
        weight = (prob >= threshold[lbl]).to(pred.device)
...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(self,
        ...
        elif self.use_thresh:
            self.cls_criterion = thresh_cross_entropy
        else:
