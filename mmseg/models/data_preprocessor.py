...
class SegDataPreProcessor(BaseDataPreprocessor):
    ...
    def __init__(
        ...
        jitter_aug: Optional[bool] = False,
        jitter_strength: Optional[float] = 0.0
    ):
        super().__init__()
        self.size = size
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        if training and self.jitter_aug:
            aug = self.get_preaugment_jitter()

            all_aug_views = []
            for img in inputs:
                augmented_views = []
                augmented_views.append(aug(img))
                all_aug_views += augmented_views
            inputs = inputs + all_aug_views 
    ...
    def get_preaugment_jitter(self):
